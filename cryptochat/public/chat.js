$(function(){
  ...
  socket.on('new_message', (data) => {
    let cl_hash = sha256(data.message);

    $('#messages').append(`<p class="message"><b>${data.username}:</b> ${data.message}
    [ client_hash: <i>${cl_hash} ] [ <span></span> ]<br>[ sign: <i>${data.s_sign}</i> || pubkey: <i>${data.s_pubkey}</i> ]</p>`);

    // make incorrect hash
    if (data.message === 'abc') {
      cl_hash = sha256('abc123');
    }
    socket.emit('verify_message', {
      cl_hash: cl_hash,
      cl_sign: data.sign,
      cl_pubkey: data.pubkey
    });
  });
